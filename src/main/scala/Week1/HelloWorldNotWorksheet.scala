...
object HelloWorldNotWorksheet {
  ...
  def stringMult(input:String, mult :Int):String ={
    temp = ""
    for( a <- 1 to mult){
      temp +="\n"+input
    }
  ...
  //task13 - STRINGS 5
  reverseString("Hello")
  def reverseString (input:String):String = input.reverse

  //task14 - OPERATORS
  def sumOfInts(num1 : Int, num2 : Int) : Int = {num1 + num2}

  //task15 - OPERATORS 2
  def ageInSeconds(age:Int):String={"you're age in seconds is " + (age*365.25*24*60*60).toLong}
  println(ageInSeconds(1))
  ...
  task27(List(1, 2, 3, 21, 22, 23))
  def task27(numb: List[Int]): List[Int]={
